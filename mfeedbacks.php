...
$isbn = $_POST['isbn'];
$query = mysql_query("select * from feedback natural join (select B.FeedBackID,(B.count/A.count)*100 as Percentage from 
                (select FeedBackID, count(FeedbackID) as count from usefulness 
            where feedbackID in (select FeedBackID from feedback where ISBN = '$isbn')
            and (FeedBack like 'Useless%' or FeedBack like 'Useful%') Group by FeedBackID order by count desc) as A,
            (select FeedBackID, count(FeedbackID) as count from usefulness 
            where feedbackID in (select FeedBackID from feedback where ISBN = '$isbn')
            and (FeedBack like 'Useful%') Group by FeedBackID order by count desc) as B
            where A.FeedBackID = B.FeedBackID order by Percentage desc limit $quantity) as D;");
?>
<!DOCTYPE html>
<html lang="en">
