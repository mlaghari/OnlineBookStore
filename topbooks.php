...
include("session.php");
    $year = $_POST['year'];
    $limit = $_POST['limit'];
    $query = mysql_query("select books.ISBN, books.Title, count as Sold from books, 
            (select A.ISBN, A.count from (select ISBN, count(*) as count from booksordered 
            where DateOrdered like '%$year%' group by ISBN order by count desc Limit $limit) as A) as C
            where books.ISBN = C.ISBN");
?>
<!DOCTYPE html>
<html lang="en">
